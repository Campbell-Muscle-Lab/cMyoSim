...
def fit_model(json_analysis_file_string):
    ...
    with open(json_analysis_file_string, 'r') as f:
        ...
        model_struct = json_data['cMyoSim_setup']['model']
    ...
    elif (fitting_struct['optimizer'] == 'differential_evolution'):
        differential_evolution(run_single, bnds,
                               (json_analysis_file_string, progress_data))
    elif (fitting_struct['optimizer'] == 'direct'):
        direct(run_single, bnds,
               args=(json_analysis_file_string, progress_data))
    elif (fitting_struct['optimizer'] == 'dual_annealing'):
        dual_annealing(run_single, bnds,
               args=(json_analysis_file_string, progress_data),
...
def pso(p_vector, json_analysis_file_string, progress_data,
        ...
        inertia = 0.5, w_self = 0.75, w_family = 0.25,
        initial_vel = 0.2,
        vel_bounds = [0.0, 20],
        vel_factor = 1,
        jitter = 0.01,
    ...
    for iter in range(100):
        ...
        thread_dir = json_data['cMyoSim_setup']['model']['fitting']['thread_folder']
        ...
        for i in range(n_particles):
            ...
            if (particle_best_value[i] < global_best_value):
                ...
                # It's a new best, make the figures and copy
...
def run_single(p_vector, json_analysis_file_string, progress_data):
    ...
    # Clean thread folder
    char_struct = json_data['cMyoSim_setup']['characterization'][0]
...
def thread_worker(pars):
    ...
    model_struct = orig_setup['cMyoSim_setup']['model']
...
def return_sim_setups(orig_setup, pars):
    ...
    new_setup['cMyoSim_setup'] = dict()
    new_setup['cMyoSim_setup']['MyoSimCpp_exe'] = \
        orig_setup['cMyoSim_setup']['MyoSimCpp_exe']
    
    # Now the model
    new_setup['cMyoSim_setup']['model'] = dict()
    new_setup['cMyoSim_setup']['model']['relative_to'] = 'False'
    new_setup['cMyoSim_setup']['model']['options_file'] = str(Path(
        os.path.join(pars['thread_space'],
                     'working',
                     orig_setup['cMyoSim_setup']['model']['options_file'])).resolve())
    
    manip = dict()
    manip['base_model'] = str(Path(
        os.path.join(pars['thread_space'],
                     'working',
                     orig_setup['cMyoSim_setup']['model']['fitting']['base_model'])).resolve())
    ...
    # Now the adjustments
    manip['adjustments'] = \
        return_adjustments(orig_setup['cMyoSim_setup']['model']['fitting'],
                           pars['x'])
        
    new_setup['cMyoSim_setup']['model']['manipulations'] = manip
        
    # Now the characterization
    new_ch = dict()
    orig_ch = orig_setup['cMyoSim_setup']['characterization'][0]
    ...
    new_setup['cMyoSim_setup']['characterization'] = []
    new_setup['cMyoSim_setup']['characterization'].append(new_ch)
    
    # Create a no-figures version
    no_figs_setup = copy.deepcopy(new_setup)
    no_figs_setup['cMyoSim_setup']['characterization'][0]['figures_off'] = 'True'
    
    # Create a figs version that creates figures and runs the
    # Python_objective call
    figs_setup = copy.deepcopy(new_setup)
    figs_setup['cMyoSim_setup']['characterization'][0]['figures_only'] = 'True'
...
def update_best_thread(progress_data, trial_df, thread_space, Python_best_call = []):
    ...
    generated_dir = str(Path(os.path.join(thread_space, 'generated')).resolve())
    ...
    sim_output_dir = str(Path(os.path.join(thread_space, 'sim_data/sim_output')).resolve())
    ...
    if (not 'Python_best_call' == []):
        best_call = str(Path(os.path.join(thread_space,
                                          '../',
                                          Python_best_call)).resolve())
