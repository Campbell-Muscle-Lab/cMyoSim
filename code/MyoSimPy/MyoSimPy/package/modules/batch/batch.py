...
def run_batch(json_batch_file_string=[],
    ...
    if (batch_structure == []):
        print('Loading cMyoSim batch from: %s' % json_batch_file_string)
        with open(json_batch_file_string, 'r') as f:
            json_data = json.load(f)
            batch_structure = json_data['MyoSim_batch']

    # Pull off the exe path
    exe_structure = batch_structure['MyoSimCpp_exe']
...
def run_multiple_batch(json_multiple_batch_file_string):
    ...
    with open(json_multiple_batch_file_string, 'r') as f:
        json_data = json.load(f)
        batch_list = json_data['MyoSim_multiple_batch']['batch_list']
