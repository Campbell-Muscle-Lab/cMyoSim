...
def characterize_model(json_analysis_file_string):
    ...
    with open(json_analysis_file_string, 'r') as f:
        ...
        anal_struct = json_data['cMyoSim_setup']
...
def generate_model_files(json_analysis_file_string):
    ...
    json_data['cMyoSim_setup']['model']['model_files'] = generated_models
    
    # Delete the adjustments
    del(json_data['cMyoSim_setup']['model']['manipulations'])
...
def deduce_pCa_length_control_properties(json_analysis_file_string,
    ...
    with open(json_analysis_file_string, 'r') as f:
        json_data = json.load(f)
        char_struct = json_data['cMyoSim_setup']
    
    # Pull off the components
    MyoSimCpp_exe_struct = char_struct['MyoSimCpp_exe']
    ...
    cpp_exe = dict()
    if ('relative_to' in MyoSimCpp_exe_struct):
        if (MyoSimCpp_exe_struct['relative_to'] == 'this_file'):
            base_dir = Path(json_analysis_file_string).parent.absolute()
        else:
            base_dir = MyoSimCpp_exe_struct['relative_to']
        cpp_exe['exe_file'] = os.path.join(base_dir,
                                           MyoSimCpp_exe_struct['exe_file'])
    else:
        cpp_exe['exe_file'] = MyoSimCpp_exe_struct['exe_file']
    ...
    pCa_lc_batch = dict()
    pCa_lc_batch['MyoSim_batch'] = pCa_lc_b
...
def deduce_fv_properties(json_analysis_file_string,
    ...
    if (fv_struct['pCa'] == 'pCa_50'):
        ...
        isometric_jobs = pCa_output['sim_batch']['MyoSim_batch']['job']
        ...
        isometric_batch['MyoSim_batch'] = isometric_b    
        ...
        isometric_jobs = isometric_batch['MyoSim_batch']['job']
    ...
    isotonic_batch = dict()
    isotonic_batch['MyoSim_batch'] = isotonic_b
...
def deduce_freeform_properties(json_analysis_file_string,
    ...
    if ('relative_to' in MyoSimCpp_exe_struct):
        ...
        cpp_exe['exe_file'] = str(Path(os.path.join(base_dir,
                                           MyoSimCpp_exe_struct['exe_file'])).resolve())
    else:
        cpp_exe['exe_file'] = MyoSimCpp_exe_struct['exe_file']

    freeform_b['MyoSimCpp_exe'] = cpp_exe
    ...
    # Now insert isometric_b into a full batch structure
    freeform_batch = dict()
    freeform_batch['MyoSim_batch'] = freeform_b
...
def characterize_fv_with_pCa_and_isometric_force(json_analysis_file_string,
    ...
    # Now insert isotonic_b into a full batch structure
    isotonic_batch = dict()
    isotonic_batch['MyoSim_batch'] = fv_dict
